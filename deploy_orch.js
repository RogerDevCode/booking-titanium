...
async function deploy() {
  const files = ['WF2_Booking_Orchestrator.json', 'WF2_Booking_Orchestrator_Error_Handler.json'];
  
  for (const file of files) {
    const wfPath = `./workflows/seed_clean/${file}`;
    if (!fs.existsSync(wfPath)) {
      console.log(`Skipping ${file}, does not exist locally yet`);
      continue;
    }

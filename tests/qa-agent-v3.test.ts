 ...
 *   - Variable WEBHOOK_BASE_URL en .env (default: https://n8n.stax.ink)
 *
 * Estructura de tests:
...
const BASE_URL = (process.env.WEBHOOK_BASE_URL || 'https://n8n.stax.ink').replace(/\/$/, '');
...
async function sendMessage(chatId: number, text: string): Promise<{
  ...
  try {
    ...
    console.log(`[DEBUG] Raw JSON from webhook:`, JSON.stringify(json, null, 2));
    // n8n puede devolver array o objeto
    const item = Array.isArray(json) ? json[0] : json;
    console.log(`[DEBUG] Extracted item:`, JSON.stringify(item, null, 2));
...
async function main() {
  ...
  await test('T11', 'DAL timeout simulado → agente responde amablemente', async () => {
    ...
    expect('no raw json').notToMatch(resp, /\{"success"/);  // sin flag /s — compatible ES2016
